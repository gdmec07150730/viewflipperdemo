package s07150730.viewflipperdemo;
...
public class MainActivity extends AppCompatActivity implements GestureDetector.OnGestureListener{
    ...
    private static final int FILNG_MIN_VELOCITY=200;
    ...
    public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
        if(e2.getX()-e1.getX()>FILNG_MIN_DISTANCE&&Math.abs(velocityX)>FILNG_MIN_VELOCITY){
            viewFlipper.setInAnimation(lInAnim);
            viewFlipper.setOutAnimation(rOutAnim);
            viewFlipper.showPrevious();
            setTitle("照片编号:"+viewFlipper.getDisplayedChild());
            return true;
        }else if(e1.getX()-e2.getX()>FILNG_MIN_DISTANCE&&Math.abs(velocityX)>FILNG_MIN_VELOCITY){
